...
import seker.common.cache.ConnectivitySupport.IConnectivityListener;
import seker.common.cache.ConnectivitySupport.NetState;
...
public class ImageLoader implements Runnable {
    ...
    public static void startConnectivityMonitor(Context context) {
        ...
        s_ConnectivityListener = new IConnectivityListener() {
            ...
            public void onConnectivityChanged(NetState oldstate, NetState newstate) {
                Log.d("l99", String.format("onConnectivityChanged(oldstate=%s, newstate=%s)", oldstate, newstate));
            }
        };
    ...
    private ImageLoader(String url, boolean limit) {
        mImageUrl = url;
        mImageLimit = limit;
    }
    ...
    public static void start(String imageUrl, ImageLoaderHandler handler, boolean limit) {
        ...
        if (null == bitmap) {
            ...
            if (null == handlers) {
                ...
                // Log.d("l99", String.format("new ArrayList<ImageLoaderHandler>(6):%s", imageUrl));
                handlers.add(handler);
                mHandlersCache.put(imageUrl, handlers);
                executor.execute(new ImageLoader(imageUrl, limit));
            } else {
                handlers.add(handler);
                // Log.d("l99", String.format("handlers.add(handler):%s", imageUrl));
            }
        } else {
    ...
    protected DownLoadResult downloadImage() {
            ...
            try {
                ...
                if (fileSize < 0) {
                    ...
                    if (mImageLimit && NetState.NET_CONNECT_MOBILE == s_ConnectivitySupport.getNetState() && fileSize > S_LIMIT) {
                        ...
                        byte[] imageData = retrieveImageData(connection, fileSize);
                        connection.disconnect();
                        if (imageData != null) {
    ...
    protected byte[] retrieveImageData(HttpURLConnection connection, int fileSize) throws IOException {
    ...
    public void notifyImageLoaded(String url, Bitmap bitmap, boolean limit) {
        ...
        // Log.i("l99", String.format("notifyImageLoaded(%d):limit=%s, %s", handlers.size(), limit, mImageUrl));
